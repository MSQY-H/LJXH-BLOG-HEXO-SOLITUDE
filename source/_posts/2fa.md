...
cover: d4.webp
comments: true
ai_text: 本文通俗介绍 2FA （双因子认证），推荐 TOTP 方式，以 GitHub 为例演示开启步骤，强调保存恢复代码。提醒 2FA 非绝对安全，需注意上网习惯。适合小白上手。
---
## ✍️写在前面

最近闲来无事，在翻 GitHub 的账号设置。翻到 `Password and authentication（密码和身份验证）` 时，看到了个 `Two-factor authentication（双重认证）` ，也就是 2FA。一查才知道，这个东西对账号安全非常重要。也就是说，开启 2FA，基本没人盗得了你的号{% spoiler 'blur' '包括你自己，但是不用担心，只要保存好恢复码，你还是能找回账号' %}
